...
# Se já estiver autenticado (teste com modelo cloud não retorna 401), não faz nada.
# Caso contrário, exibe o link de conexão e OBRIGA o usuário a confirmar que concluiu antes de continuar.
# Uso: chamado por make up após aplicar Ollama (ou manualmente: ./scripts/ollama-ensure-cloud-auth.sh).
set -e
...
is_ollama_ready() {
  kubectl get deploy ollama-gpu -n "$NAMESPACE" &>/dev/null && \
  kubectl rollout status deployment/ollama-gpu -n "$NAMESPACE" --timeout=0 &>/dev/null
}
...
if [[ -z "$URL" ]]; then
  echo "    Não foi possível obter o link de conexão. Execute manualmente: ./scripts/ollama-signin.sh"
  exit 1
fi

echo "------------------------------------------------------------------------------"
echo "  AÇÃO OBRIGATÓRIA: Abra o link abaixo no navegador e conclua o login."
echo "  (Faça login em ollama.com se precisar e autorize o dispositivo.)"
echo ""
echo "  $URL"
echo ""
echo "------------------------------------------------------------------------------"
...
echo "    ou rode novamente: ./scripts/ollama-ensure-cloud-auth.sh"
exit 1
