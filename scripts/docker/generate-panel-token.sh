...
LOGIN_RESP=$(curl -sf --connect-timeout 2 --max-time 10 -X POST "$BACKEND/auth/login" \
...
AGENT_RESP=$(curl -sf --connect-timeout 2 --max-time 10 -X POST "$BACKEND/auth/agent-token" \
