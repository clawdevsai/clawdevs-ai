...
| **CEO fecha e pergunta ao Diretor** | CEO consolida, pergunta deploy staging, etc. | **Prompt** | Descrito no SOUL do CEO (`config/openclaw/workspace-ceo/SOUL.md`). Comportamento depende do LLM; não há código que force esse passo. |
...
| **App no canal** | App ClawdevsAI no canal | **Config** | Documentado em `config/openclaw/README.md` (adicionar app ao canal). Não há código que adicione o app; é passo manual no Slack. |
| **Rodada: um agente por vez (DevOps → … → CEO)** | Ordem fixa; cada um opina na sua área | **Só no prompt** | A ordem e o “um por vez” estão **apenas** em `config/openclaw/workspace-ceo/SOUL.md` (instruções ao CEO). Não existe neste repo código que garanta ordem ou exclusão (ex.: lock por rodada, máquina de estados). O comportamento depende do modelo seguir o SOUL. |
