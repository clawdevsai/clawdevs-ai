...
class RequirementParser {
  ...
  private parseFromConversation(text: string): Requirements | null {
    ...
    if (/1[.,]?0*\s*[mM]\s*(?:requests|rps|reqs)?/.test(text)) {
      ...
      hasMatch = true;
    } else if (/(?:scale|volume|throughput|load)/i.test(text) && /(?:small|minimal|low)/i.test(text)) {
    ...
    if (/99\.99|four\s*nines|highly[\s-]critical|financial|healthcare/.test(text)) {
      ...
      hasMatch = true;
    } else if (/best[\s-]?effort|95%|eventual|best[\s-]?effort/.test(text)) {
    ...
    // COMPLEXITY parsing
    if (
      /(?:machine\s*learning|realtime\s*analytics|streaming\s*data|ml\s*pipeline|graph[\s-]?db)/i.test(text)
    ) {
      requirements.dataComplexity = 'extreme';
      hasMatch = true;
    } else if (/(?:complex|relationships|joins|graph[\s-]?like|interconnected|highly[\s-]?related)/i.test(text)) {
