...
.PHONY: help test clean check-runtime-stack preflight gpu-host-check gpu-cdi-check gpu-cdi-help minikube-start gpu-wait gpu-assert gpu-debug image-build deploy up up-force up-cdi down restart status logs gpu-smoke ollama-pull cluster-reset cluster-reset-hard
...
help:
	...
	@echo "make up        - sobe stack completa no Minikube com GPU (modo all)"
